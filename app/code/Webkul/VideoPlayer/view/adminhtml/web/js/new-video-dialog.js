 ...
 */
 define([
    ...
    'Webkul_VideoPlayer/js/get-video-information',
    'mage/cookies'
], function ($, _) {
    'use strict';
    ...
    $.widget('mage.createVideoPlayer', {
        ...
        _doUpdate: function () {
            ...
            this.reset();
            if (this.options.metaData.data.title != undefined && this.options.metaData.data.title != "") {
            ...
            if (this.options.videoProvider === 'youtube') {
                uploaderLinkUrl = 'https://youtube.com/embed/' + this.options.videoId;
            } else if (this.options.videoProvider === 'vimeo') {
                uploaderLinkUrl = this.options.metaData.data.uploaderUrl;
            } else if (this.options.videoProvider === 'upload') {
                uploaderLinkUrl = this.options.metaData.data.uploaderUrl?this.options.metaData.data.uploaderUrl:this.options.videoId;
                uploaderLink = document.createElement('input');
                uploaderLink.setAttribute('id', 'plyr-url');
                uploaderLink.setAttribute('type', 'hidden');
                uploaderLink.value = uploaderLinkUrl;
    ...
    $.widget('mage.updateInputFields', {
        options: {
            ...
            DOM: {
                ...
                videoSource: 'input[name="video_source"]',
        ...
        update: function () {
            ...
            $(this.options.DOM.videoSource).val(this.options.data.source);
        },
        ...
        reset: function () {
            ...
            $(this.options.DOM.videoSource).val('');
    ...
    $.widget('mage.wkNewVideoDialog', {
        ...
        _videoUploadInputSelector: '#video_file',
        ...
        _videoUploadUrlSelector: '#upload-video-url',
        ...
        _onImageLoaded: function (result, file, oldFile, callback) {
            ...
            $.each(this.element.find(this._videoFormSelector).serializeArray(), function (i, field) {
                ...
                if (data['video_source'] == 'upload') {
                    data['video_url'] = self.element.find(self._videoUploadUrlSelector).val();
                }
            });
        ...
        _create: function () {
            ...
            videoUploader = this.element.find(this._videoUploadInputSelector);
            videoUploader.on('change', this._onVideoInputChange.bind(this));
            videoUploader.attr('accept', this._videoTypes.join(','));
            videoUploader.after('<span class="file-uploader-spinner"></span>');
            ...
            this.element.modal({
                ...
                opened: function () {
                    ...
                    source = widget.element.find('#video_source').val();
                    ...
                    $('#new_video_screenshot').removeClass('required-entry _required');
        ...
        _onVideoInputChange: function () {
            var jFile = this.element.find(this._videoUploadInputSelector),
                file = jFile[0],
                val = jFile.val(),
                ext = '.' + val.split('.').pop();
            ...
            jFile.parent().addClass('file-uploader').addClass('_loading');
            this._uploadVideoFile(file, jFile);
        },
         ...
         * @param {*} file 
         */
        _uploadVideoFile: function (file, jFile) {
            var self = this;
            var fd = new FormData();
            var files = file.files[0];
            fd.append('file', files);
            fd.append('form_key', this.options.formKey);
            $.ajax({
                url: this.options.saveVideoFileUrl,
                type: 'post',
                data: fd,
                contentType: false,
                processData: false,
                success: function(response){
                    jFile.parent().removeClass('_loading');
                    var modal = self.element.closest('.mage-new-video-dialog');
                    modal.createVideoPlayer({
                        videoId: response.size,
                        ...
                        metaData: {
                            ...
                            data: {
                                title: response.name,
                                uploaderUrl: response.uploadedUrl,
                                uploader: 'video file'
                            }
                        }
                    });
                    self.element.find(self._videoUrlSelector).val(response.uploadedUrl).trigger('focusout');
                    self.element.find(self._videoUploadUrlSelector).val(response.uploadedUrl);
        ...
        _onOpenDialog: function (e, imageData) {
            ...
            if (imageData['media_type'] === 'external-video') {
                ...
                $.each(formFields, function (i, field) {
                    ...
                    if (field.name == 'video_source' && imageData[field.name] == 'url') {
                        self.element.find(self._videoUploadInputSelector).hide();
                        self.element.find('.field-video_file').hide();
                        self.element.find(self._videoUrlSelector).show().attr('disabled', false);
                        self.element.find('.field-video_url').show();
                    }
                });
