        ...
        try {
            if ($isPost) {
                ...
                foreach ($importProductRawData as $rowIndex => $dataRow) {
                    if ((isset($dataRow[0]) && isset($dataRow[1])) && trim($dataRow[0]) != 'Zipcode' && trim(
                            $dataRow[1] != 'City'
                        ) && $counter == 1) {
                    ...
                    if ($counter > 1 && (isset($dataRow[0]) && isset($dataRow[1])) && $dataRow[0] != "" && $dataRow[1] != "") {
                        ...
                        $_cities = $_cities->addFieldToFilter('city', trim($dataRow[1]))->addFieldToFilter(
                            'state_id',
                            $state_id
                        )
                        ...
                        if ($_cities->count() > 0) {
                            ...
                            $this->zipCollection->clear()->getSelect()->reset(Select::WHERE);
                            ...
                            if ($zip_check->count() == 0) {
                                ...
                                $not_exists .= '<br />' . __('Zip code') . ' <b>"' . $dataRow[0] . '"</b> ' . __(
                                        'is already exists.'
                                    );
                            }
                        } else {
                            $not_exists .= '<br />' . __('City') . ' <b>"' . $dataRow[1] . '"</b> ' . __(
                                    'is not exists in selected country.'
                                );
                        }
                    }
                    $counter++;
                ...
                if ($import > 0) {
                    ...
                    $this->messageManager->addError(
                        __(
                            'No zip code imported, either already exists or data is not correct, check your file.'
                        ) . $not_exists
                    );
                }
            }
