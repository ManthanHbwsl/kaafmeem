 ...
 */
class BalanceAfter extends \Magento\Framework\View\Element\Template
{
    /**
    ...
    public function getPayoutAmount($params)
    {
        if ($this->payout === null) {
            ...
            $this->payout = $profitReport->getPayoutCompleted($params->getMemberId(), $params->getTo(), '<=');
        }
        return $this->payout;
    }
    ...
    public function getLifeTimeAmount($params)
    {
        if ($this->lifeTimeAmount === null) {
            /** @var \BrainActs\SalesRepresentative\Model\ResourceModel\Report\Profit $profitReport */
            $profitReport = $this->profitFactory->create();
            $this->lifeTimeAmount = $profitReport->getLifeTimeCommissions(
                $params->getMemberId(),
                $params->getTo(),
                '<='
            );
