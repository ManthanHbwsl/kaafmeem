...
class SelectService implements SelectServiceInterface
{
    ...
    public function createTemporaryTable(ColumnInterface $column, RequestInterface $request, TableInterface $baseTable)
    {
        ...
        $reportRepository = $this->objectManager->get(ReportRepository::class);

        $report = $reportRepository->get($request->getIdentifier());

        $select->where($baseTable->getPkField()->toDbExpr() . '>0');

        $allowedFiltersForTmp = [];

        if ($report) {
            $allowedFiltersForTmp = $report->getPrimaryFilters();

            foreach ($report->getInternalFilters() as $internalFilter) {
                $allowedFiltersForTmp[] = $internalFilter['column'];
            }
        }
        ...
        foreach ($request->getFilters() as $filter) {
            ...
            if ($report && !in_array($filter->getColumn(), $allowedFiltersForTmp)) {
                continue;
            }
...
//                echo $insertQuery.PHP_EOL.PHP_EOL.PHP_EOL.PHP_EOL;
    ...
    {
        // for properly applying child item pill
        if(
            $baseTable->getName() == 'sales_order_item'
            && $column->getTable()->getName() == 'sales_order_item'
            && $column->getName() !== 'item_id'
        ) {
            return true;
        }
