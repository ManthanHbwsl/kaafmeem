...
class Archive extends Action
{
    ...
    public function execute()
    {
        ...
        try {
            $this->_action->archiveOrder($orderId);
            $this->messageManager->addSuccessMessage(__('The order has been archived.'));
        } catch (Exception $e) {
            $this->messageManager->addErrorMessage(
                __('An error occurred while archiving the order. Please try again later. %1', $e->getMessage())
            );
