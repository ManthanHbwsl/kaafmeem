 ...
 */
class MassUnarchive extends AbstractMassAction
{
    /**
    ...
    protected function massAction(AbstractCollection $collection)
    {
        ...
        foreach ($collection->getItems() as $order) {
            try {
                $this->_action->unarchiveOrder($order->getId());
                $archived++;
            } catch (Exception $e) {
                $this->messageManager->addErrorMessage(
                    __(
                        'Cannot unarchive order #%1. Please try again later. %2',
        ...
        if ($archived) {
            $this->messageManager->addSuccessMessage(__('A total of %1 order(s) has been unarchived.', $archived));
        }
