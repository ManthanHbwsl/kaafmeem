<?php
/**
 * Mageplaza
 *
 * NOTICE OF LICENSE
 *
 * This source file is subject to the Mageplaza.com license that is
 * available through the world-wide-web at this URL:
 * https://www.mageplaza.com/LICENSE.txt
 *
 * DISCLAIMER
 *
 * Do not edit or add to this file if you wish to upgrade this extension to newer
 * version in the future.
 *
 * @category    Mageplaza
 * @package     Mageplaza_GiftWrap
 * @copyright   Copyright (c) Mageplaza (https://www.mageplaza.com/)
 * @license     https://www.mageplaza.com/LICENSE.txt
 */

namespace Mageplaza\GiftWrap\Controller\Adminhtml\Category;

use Magento\Backend\Model\View\Result\Page;
use Magento\Framework\App\ResponseInterface;
use Magento\Framework\Controller\ResultInterface;
use Mageplaza\GiftWrap\Controller\Adminhtml\AbstractCategory;

/**
 * Class Edit
 * @package Mageplaza\GiftWrap\Controller\Adminhtml\Category
 */
class Edit extends AbstractCategory
{
    /**
     * @return Page|ResponseInterface|ResultInterface
     */
    public function execute()
    {
        $object = $this->_initCategory();

        if ($objId = $this->getRequest()->getParam('id')) {
            $object->load($objId);

            if (!$object->getId()) {
                $this->messageManager->addErrorMessage(__('This category no longer exists.'));

                return $this->_redirect('*/*/');
            }
        }

        // restore form data
        $data = $this->_session->getCategoryData();
        if (!empty($data)) {
            $object->addData($data);
        }

        $this->registry->register('mpgiftwrap_category', $object);

        if ($object) {
            $pageTitle = $objId ? __('View Category #%1', $objId) : __('Create New Wrap Category');
            $resultPage = $this->_initAction();
            $resultPage->getConfig()->getTitle()->prepend($pageTitle);

            return $resultPage;
        }

        return $this->_redirect('*/*/');
    }
}
